...
Detik.prototype.getURL = function($) {
  var url = undefined
  url = (_.isUndefined(url) || url.length < 10) ? $('meta[property="og:url"]').attr('content') : url
  url = (_.isUndefined(url) || url.length < 10) ? $('link[rel="canonical"]').attr('href') : url
  if (_.isUndefined(url) || url.length < 10) {
    let urls = $('link[rel="canonical"]')
      .get()
      .map((el, i) => $(el).attr('href'))
